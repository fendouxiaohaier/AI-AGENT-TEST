...
`),
  ...
  for (let i = 0; i < maxIterations; i++) {
    ...
    console.log(
      chalk.bold.red(`--- 第${i + 1}次调用AI ---\n`),
      // chalk.bold.red(`当前最新调用的messages是：\n`),
      // 将 messages 对象转为格式化的 JSON 字符串再染色
      // chalk.cyan(JSON.stringify(messages, null, 2)),
      // "\n",
      chalk.bold.red(`返回的response是：\n`),
      // 将 response 对象转为格式化的 JSON 字符串再染色
      chalk.green(JSON.stringify(response, null, 2)),
      // "\n",
    );
    ...
    console.log(
      chalk.bold.red(
        `找到了${response.tool_calls.length}个工具调用，开始执行...}\n`,
      ),
    );
    // 执行工具调用
    for (const toolCall of response.tool_calls) {
      ...
      if (foundTool) {
        console.log(
          chalk.bold.blue(
            `  在for循环里调用工具[工具调用名称] ${toolCall.name}`,
          ),
        );

...
const readFileTool = tool(
  async ({ filePath }) => {
    try {
      ...
      console.log(
        `  [readFileTool工具调用] read_file("${filePath}") - 成功读取 ${content.length} 字节`,
      );
      return `读取的文件内容:\n${content}`;
    } catch (error) {
      console.log(
...
const writeFileTool = tool(
  async ({ filePath, content }) => {
    try {
      ...
      console.log(
        `  [writeFileTool工具调用] write_file("${filePath}") - 成功写入 ${content.length} 字节`,
      );
...
const listDirectoryTool = tool(
  async ({ directoryPath }) => {
    try {
      ...
      console.log(
        `  [listDirectoryTool工具调用] list_directory("${directoryPath}") - 找到 ${files.length} 个项目`,
      );
      return `目录内容:\n${files.map((f) => `- ${f}`).join("\n")}`;
    } catch (error) {
      console.log(

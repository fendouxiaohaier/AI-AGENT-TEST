...
const agentStepChain = RunnableSequence.from([
  ...
  RunnableBranch.from([
    [
      (state) =>
        !state.response?.tool_calls || state.response.tool_calls.length === 0, // 如果没有 tool_calls，则直接返回 LLM 的输出
      new RunnableLambda({
        func: async (state) => {
...
await runAgentWithTools(
  "北京南站附近的酒店，最近的 1 个酒店，拿到酒店图片，打开浏览器，展示每个酒店的图片，每个 tab 一个 url 展示，并且在把那个页面标题改为酒店名",
);
